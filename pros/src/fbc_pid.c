...
static int
_pidCompute(fbc_t *fbc, int error)
{
...
void
fbcPIDInitializeData(fbc_pid_t *fbc_pid, double kP, double kI, double kD, int minIntegral, int maxIntegral)
{
    fbc_pid->kP = kP;
    fbc_pid->kI = kI;
    fbc_pid->kD = kD;
...
void
fbcPIDInit(fbc_t *fbc, fbc_pid_t *config)
{
