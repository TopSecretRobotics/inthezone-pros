...
{
    return (int16_t)motorGet((unsigned char)(index + 1));
}

static inline void
vexMotorSet(int16_t index, int16_t value)
{
    (void)motorSet((unsigned char)(index + 1), (int)value);
    return;
}
...
rpcRecvWriteCassette(rpc_t *rpc, const message_write_t *write)
{
    ...
    case MESSAGES_TOPIC_CASSETTE_SUBTOPIC_CLOSE:
        ...
        rpc->cassette = 0xff;
        // uint8_t writeval = 0;
        // (void)fwrite(&writeval, 1, 1, rpc->fp);
        (void)fflush(rpc->fp);
        (void)fclose(rpc->fp);
        rpc->fp = NULL;
        return;
    case MESSAGES_TOPIC_CASSETTE_SUBTOPIC_WRITE:
        ...
        wbuf += 1;
        (void)sdAsynchronousWrite(rpc->fp, wbuf, write->len - 1);
        // (void)fflush(rpc->fp);
        // (void)fwrite(wbuf, 1, write->len - 1, rpc->fp);
        if (fflush(rpc->fp) == EOF) {
            rpc->cassette = 0xff;
            (void)fclose(rpc->fp);
            rpc->fp = NULL;
        }
