...
void
blrsMotorInit(int port, bool inverted, float slewrate, int (*recalculate)(int))
{
    if (port < 1 || port > 10)
        return;
    port--;
...
bool
blrsMotorSet(int port, int commanded, bool immediate)
{
    if (port > 10 || port < 1)
        return false;
    if (commanded > 127)
